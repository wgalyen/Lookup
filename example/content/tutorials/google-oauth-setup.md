...
_Make sure you first [install Lookup dependencies](http://github.io/wgalyen/lookup/install/installing-lookup) after you clone it._
...
+ Add passport: `var passport=require('passport');` just after lookup is required.
+ Add oauth2 middleware: `var oauth2= require('./middleware/oauth2.js');` in the config block, just after `error_handler.js` middleware.
+ Change `secret` to `secret:config.secret,` in the `// HTTP Authentication` section.
+ >>> Remove the lk-login route `app.post('/lk-login', route_login);`
+ >>> Remove the logout route: `app.get('/logout', route_logout);`
+ Add the following Oauth settings, just before the `app.post('/lk-login', route_login);` line:
...
We added two scenarios for when we have Google OAuth enabled (`config.googleoauth`) and when we don't (defaulting to the current Lookup behavior).
...
Same thing here as well. If we have Google OAuth enabled (`config.googleoauth`) then we show the new Google login button and hide the rest. Otherwise, we default it to the current Lookup behavior.

## Testing
Congratulations, you're done! Now, to test this locally just run the `npm start` from the root of your project and go to `http://localhost:3000` and you should see a popup modal with `Sign in with Google`.

After logging in, you should see the homepage.
