...
function initialize (config) {
  ...
  if (config.authentication === true || config.authentication_for_edit || config.authentication_for_read) {
    ...
    router.post('/lk-login', route_login);
  ...
  if (config.allow_editing === true) {
    if (config.googleoauth === true) {
      router.post('/lk-edit',         oauth2.required, route_page_edit);
      router.post('/lk-delete',       oauth2.required, route_page_delete);
      router.post('/lk-add-page',     oauth2.required, route_page_create);
      router.post('/lk-add-category', oauth2.required, route_category_create);
    }
    else {
      router.post('/lk-edit',         authenticate, route_page_edit);
      router.post('/lk-delete',       authenticate, route_page_delete);
      router.post('/lk-add-page',     authenticate, route_page_create);
      router.post('/lk-add-category', authenticate, route_category_create);
    }
  }
